...
# 0: not ready yet (arrival), 1: in arrival pool, 2: added to arrival queue, 3: in service, 6: finished.
...
class FlightInfo:
    ...
    pool_time: Optional[float] = None # 9 actual time that they join the pool (generated in advance)
    release_time: Optional[float] = None # time at which aircraft is released from pool
    enters_service: Optional[float] = None # the time at which aircraft enters service
    weather_state: Optional[WeatherStatus] = None # 12 weather state at the time of release # JF Note - could possibly be removed with refactoring
    travel_time: Optional[float] = None # travel time (generated in advance), # JF Question note from entering pool to runway? Update_ETAs suggests this is the case
    travel_time_indicator: bool = False # 11 indicator to show whether or not the AC's travel time has already been completed - used in Update_ETAs
    service_time: Optional[float] = None # 8 actual service time s1+Z2 (worked out after class information is known)
    pred_cost: Optional[float] = None # 15 predicted total cost at time of release
    service_completion_time: Optional[float] = None # actual service completion time


    @classmethod
    def from_flight_data(cls, data: FlightData, ps_time: float, pax_weight: float):
        return cls(data.flight_id, data.ac_class, data.dep_sched, data.arr_sched,
...
def read_flight_data(data_fn: str, min_time: int, max_time: int, wiener_sig: float) -> tuple[list[FlightData], list[float], list[float], list[float]]:
    ...
    Returns:
    ---------
    flight_data: data (flight ID, scheduled times and class) associated to each flight
    ...
    with open(data_fn, 'r') as csvfile: # data file includes the on-time performance data such as means, variance of lateness based on 1 year of historical info from FlightRadar [DON'T CHANGE THIS FILE]
        ...
        for i in range(1, n_lines): # start from 1 because there's a title row
            arr_time = float(inputdata[i][1]) # pre-scheduled time
            ...
            if arr_time >= min_time and arr_time < max_time:
                ## Read Data ##
                ...
                h += -min_time+60
                arr_time += -min_time+60
                
                data = FlightData(flight_name, ac_class, h, arr_time)
                ...
                xibar = arr_time + lateness_mn # I.e. mean arrival time
                si2 = lateness_var 
            ...
            elif arr_time >= max_time: # Indicates that we have got to the end of the set of flights scheduled to arrive by 2PM
                break
    return flight_data, Alpha_Ps, Beta_Ps, late_means
